...
import { setPaginate } from "../sroutesSlice";
...
export const SroutesList = () => {
    const { sroutess, paginateParams } = useSelector((state) => state.sroutes);
    ...
    const [columns, setColumns] = useState(getData(sroutesPayload.columnsName) == null ? sroutesPayload.columns : getData(sroutesPayload.columnsName));

    const sroutesStatus = useRef(['ALL','ACTIVE','INACTIVE']);
    ...
    const reloadData = () =>{
        ...
        dispatch(setPaginate(sroutesPayload.paginateParams));
    }

    const deleteData = async () => {
        setIsLoading(true)
        const result = await sroutesService.destory(dispatch, selectedId)
    ...
    const loadingData = useCallback(async () => {
        const result = await sroutesService.index(dispatch, paginateParams);
        if (result.status === 200) {
            setTotal(
                result.data.total
            );
        ...
        setIsLoading(false)
        if(getData(sroutesPayload.columnsName) == null){
            setData(sroutesPayload.columnsName, sroutesPayload.columns)
        }
    ...
    useEffect(()=>{
        setData(sroutesPayload.columnsName, columns)
    },[columns])
                                        ...
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignTransferItems="center">
                                                ...
                                                <Grid transferItem xs={1}>
                                                    <TableCustomizeSetting payload={sroutesPayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>

                                                <Grid transferItem xs={2}> 
                                                    <FilterByStatus paginateParams={paginateParams} status={sroutesStatus} onFilter={onFilter} />
                                                </Grid>
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.sroutes}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
