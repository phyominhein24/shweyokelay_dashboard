...
export const MemberList = () => {
    ...
    const [columns, setColumns] = useState(getData(memberPayload.columnsName) == null ? memberPayload.columns : getData(memberPayload.columnsName));
    ...
    const deleteData = async () => {
        setIsLoading(true)
        const result = await memberService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await memberService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await memberService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await memberService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await memberService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await memberService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        const result = await memberService.index(dispatch, paginateParams);
        if (result.status === 200) {
            setTotal(
                result.data.total
            );
        ...
        setIsLoading(false)
        if(getData(memberPayload.columnsName) == null){
            setData(memberPayload.columnsName, memberPayload.columns)
        }
    ...
    useEffect(()=>{
        setData(memberPayload.columnsName, columns)
    },[columns])
                                        ...
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignTransferItems="center">
                                                ...
                                                <Grid transferItem xs={1}>
                                                    <TableCustomizeSetting payload={memberPayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.member}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
