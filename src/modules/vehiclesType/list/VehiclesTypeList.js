...
export const VehiclesTypeList = () => {
  ...
  const [columns, setColumns] = useState(
    getData(vehiclesTypePayload.columnsName) == null
      ? vehiclesTypePayload.columns
      : getData(vehiclesTypePayload.columnsName)
  );
                      ...
                      >
                        <Grid item xs={1}>
                          <TableCustomizeSetting
                            payload={vehiclesTypePayload.columns}
                            columns={columns}
                          ...
                          const switchCase = ({ column, value }) => {
                            switch (column.id) {
                              ...
                              case "option":
                                return (
                                  <NavigateId
                                    url={`${paths.vehiclesType}/${row.id}`}
                                    id={row.id}
                                  />
                                );
                              case "facilities":
                                const facilitiesValue = Array.isArray(value)
                                  ? value
                                  : JSON.parse(value || "[]");
                                return facilitiesValue.join(", ");
