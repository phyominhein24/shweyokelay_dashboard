...
import { setPaginate } from "../paymentSlice";
...
export const PaymentList = () => {
    ...
    const [columns, setColumns] = useState(getData(paymentPayload.columnsName) == null ? paymentPayload.columns : getData(paymentPayload.columnsName));

    const paymentStatus = useRef(['ALL','ACTIVE','INACTIVE']);
    ...
    const reloadData = () =>{
        ...
        dispatch(setPaginate(paymentPayload.paginateParams));
    }

    const deleteData = async () => {
        setIsLoading(true)
        const result = await paymentService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await paymentService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await paymentService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await paymentService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await paymentService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await paymentService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        const result = await paymentService.index(dispatch, paginateParams);
        if (result.status === 200) {
            setTotal(
                result.data.total
            );
        ...
        setIsLoading(false)
        if(getData(paymentPayload.columnsName) == null){
            setData(paymentPayload.columnsName, paymentPayload.columns)
        }
    ...
    useEffect(()=>{
        setData(paymentPayload.columnsName, columns)
    },[columns])
                                    ...
                                    <TableCell colSpan={12}>
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignItems="center">
                                                ...
                                                <Grid item xs={1}>
                                                    <TableCustomizeSetting payload={paymentPayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>

                                                <Grid item xs={2}> 
                                                    <FilterByStatus paginateParams={paginateParams} status={paymentStatus} onFilter={onFilter} />
                                                </Grid>
